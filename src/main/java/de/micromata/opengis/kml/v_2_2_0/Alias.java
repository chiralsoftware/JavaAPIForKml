...
public class Alias
     ...
     * <targethref> (required)
     * <p>
     * A URL that specifies the .kml or .kmz file whose data (within Google
     * Earth) is to be modified by an <Update> element. This KML file must
     * already have been loaded via a <NetworkLink>. In that file, the element
     * to be modified must already have an explicit id attribute defined for it.
     * </p>
     * <p>
     * Specifies the texture file to be fetched by Google Earth. This reference
     * can be a relative reference to an image file within the .kmz archive, or
     * it can be an absolute reference to the file (for example, a URL).
     * </p>
     *
     *
     *
     ...
     * This reference can be a relative reference to an image file within the
     * .kmz archive, or it can be an absolute reference to the file (for
     * example, a URL). <sourceHref> Is the path specified for the texture file
     * in the Collada .dae file.
     * </p>
     * <p>
     * In Google Earth, if this mapping is not supplied, the following rules are
     * used to locate the textures referenced in the Collada (.dae) file: No
     * path: If the texture name does not include a path, Google Earth looks for
     * the texture in the same directory as the .dae file that references it.
     * Relative path: If the texture name includes a relative path (for example,
     * ../images/mytexture.jpg), Google Earth interprets the path as being
     * relative to the .dae file that references it. Absolute path: If the
     * texture name is an absolute path (c:\mytexture.jpg) or a network path
     * (for example, http://myserver.com/mytexture.jpg), Google Earth looks for
     * the file in the specified location, regardless of where the .dae file is
     * located.
     * </p>
     * <p>
